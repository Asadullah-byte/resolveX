...
export const assignEngineerToError = async (req, res) => {
  try {
    ...
    const error = file.groqResponse.anomalies.find(
      (anomaly) =>
        `${anomaly.anomaly_type}_${anomaly.log_entry.substring(0, 10)}` ===
        errorId
    );
    if (!error)
      return res
    ...
    const assignment = new Assignment({
      ...
      messages: [
        {
          senderId: clientId,
          message: "You have been assigned to resolve this issue.",
        },
      ],
    });
...
export const getErrorAssignments = async (req, res) => {
  try {
    ...
    const query =
      role === "Client" ? { clientId: userId } : { engineerId: userId };
    ...
    const enriched = await Promise.all(
      assignments.map(async (a) => {
        ...
        const error = file?.groqResponse?.anomalies?.find(
          (an) =>
            `${an.anomaly_type}_${an.log_entry.substring(0, 10)}` === a.errorId
        );
        return {
          ...a,
          ...
          repoUrl: a.repoUrl,
          repoLinked: !!a.repoAccessToken, // or manually managed boolean
        };
      })
    );
...
export const getAssignmentDetails = async (req, res) => {
  try {
    ...
    const error = file?.groqResponse?.anomalies?.find(
      (an) =>
        `${an.anomaly_type}_${an.log_entry.substring(0, 10)}` ===
        assignment.errorId
    );
...
export const getChatMessages = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      messages: assignment.messages.sort((a, b) => a.sentAt - b.sentAt),
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
