...
import { uploadedFile } from "../models/uploadedFile.js"; // ✅ Fixed model import
import { sendToGroq } from "../services/groqService.js";
import { prisma } from "../../../../db/connectDB.js"; // ✅ Fixed Prisma import
...
export const uploadFile = async (req, res) => {
  try {
    ...
    const existingProcessingFile = await uploadedFile.findOne({ status: "Processing" });
...
export const processNextFile = async () => {
  try {
    ...
    const fileContent = fs.readFileSync(path.join("public", fileToProcess.filePath), "utf8");
    console.log("📂 Read file content successfully:", fileContent.substring(0, 200));

    const groqResponse = await sendToGroq(fileContent);

    fileToProcess.status = "Analyzed"; // ✅ Updated to "Analyzed"
